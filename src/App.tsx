...
/**
 * RANGES — update these with exact min/max from brand category pages.
 * Quantities are fixed (projection-only).
 */
const RANGES: Room[] = [
  {
    key: "living",
...
/** Tier math: Good = min; Better = slightly above midpoint; Best = max */
const unitGood = (r: Range) => r.min;
const unitBest = (r: Range) => r.max;
const unitBetter = (r: Range) => Math.round(r.min + 0.55 * (r.max - r.min));
...
export default function App() {
  ...
  const lines = room.items.map(i => {
    ...
    const good = unitGood(r) * i.qty;
    const better = unitBetter(r) * i.qty;
    const best = unitBest(r) * i.qty;
    return { ...i, r, good, better, best };
  });

  const totals = lines.reduce(
    (acc, l) => ({ good: acc.good + l.good, better: acc.better + l.better, best: acc.best + l.best }),
    { good: 0, better: 0, best: 0 }
  );

  const box: React.CSSProperties = { padding: 16, border: "1px solid #e5e5e5", borderRadius: 12, background: "#fff" };
  const head = { display: "grid", gridTemplateColumns: "1.2fr 0.6fr 1fr 1fr 1fr", gap: 8, fontSize: 12, fontWeight: 600, color: "#555", paddingBottom: 8, borderBottom: "1px solid #eaeaea" };
  const row = { display: "grid", gridTemplateColumns: "1.2fr 0.6fr 1fr 1fr 1fr", gap: 8, padding: "10px 0", borderTop: "1px solid #f0f0f0", alignItems: "center" };

  return (
    <div style={{ fontFamily: "Inter, system-ui, sans-serif", margin: "0 auto", maxWidth: 1100, padding: 24, background: "#f7f7f9", minHeight: "100vh" }}>
      <h1 style={{ fontSize: 24, marginBottom: 4 }}>Client Budget Projection — Good / Better / Best</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 16 }}>
        ...
        <div style={box}>
          ...
          <div style={{ marginTop: 14, fontSize: 12, color: "#777" }}>
            Tip: Replace min/max in <code>RANGES</code> with live site values to keep projections current.
          </div>
        </div>

        {/* Table */}
        <div style={box}>
          <div style={head as any}>
            <div>Item</div>
            <div style={{ textAlign: "right" }}>Qty</div>
            <div style={{ textAlign: "right" }}>Good</div>
            <div style={{ textAlign: "right" }}>Better</div>
            <div style={{ textAlign: "right" }}>Best</div>
          </div>

          {lines.map(l => (
            <div key={l.key} style={row as any}>
              <div>{l.label}</div>
              <div style={{ textAlign: "right" }}>{l.qty}</div>
              <div style={{ textAlign: "right" }}>{currency(l.good)}</div>
              <div style={{ textAlign: "right", fontWeight: 600 }}>{currency(l.better)}</div>
              <div style={{ textAlign: "right" }}>{currency(l.best)}</div>
            </div>
          ))}

          <div style={{ borderTop: "1px solid #eaeaea", marginTop: 12, paddingTop: 12 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1.2fr 0.6fr 1fr 1fr 1fr", gap: 8 }}>
              <div style={{ fontWeight: 700 }}>Room Projection Total</div>
              <div />
              <div style={{ textAlign: "right", fontWeight: 700 }}>{currency(totals.good)}</div>
              <div style={{ textAlign: "right", fontWeight: 800 }}>{currency(totals.better)}</div>
              <div style={{ textAlign: "right", fontWeight: 700 }}>{currency(totals.best)}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <p style={{ color: "#777", fontSize: 12, marginTop: 12 }}>
        “Good” ≈ lowest current list prices. “Better” ≈ midpoint leaning premium. “Best” ≈ highest current list prices.
      </p>
    </div>
  );
}
